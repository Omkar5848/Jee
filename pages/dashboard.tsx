...
type Role = 'doctor' | 'patient' | 'technician' | 'nurse' | 'admin' | 'receptionist';
type User = { name: string; email: string; profession: Role };
...
export default function Dashboard() {
  ...
  const theme = useMemo(() => {
    if (!user) return roleTheme.technician;
    return roleTheme[user.profession] || roleTheme.technician;
  }, [user]);
    ...
    <div className="dashboardPage">
      <div className="dashboardShell">
        <div className="overviewBanner" style={{ borderColor: theme.color }}>
          ...
          <button className="button" style={{ width: 'auto', padding: '10px 18px' }} onClick={logout}>Logout</button>
        </div>
